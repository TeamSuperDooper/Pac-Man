...
public class GhostMove : MonoBehaviour {
   ...
   // private GameManager _gm;
    ...
    void FixedUpdate() {
        Debug.Log("1. Fixed update Ghost move");
        ...
        //if (GameManager.gameState == GameManager.GameState.Game) {
            ...
            switch (state) {
                ...
                case State.Scatter:
                    Debug.Log("Scattering (switch)");
                    Scatter();
                    break;

                case State.Chase:
                    Debug.Log("Chase switch in Fixed update ghost move");
                    ChaseAI();
                    break;
    ...
    public void InitializeGhost() {
        ...
        state = State.Wait;
        timeToEndWait = Time.time + waitLength + 1; //GUINav.initialDelay;
...
10 8";
        ...
        if (st == State.Wait) {
            ...
            if (transform.name == "inky" || transform.name == "clyde") {
                waypoints.Enqueue(new Vector3(pos.x, 0f, pos.z - 0.5f));
                waypoints.Enqueue(new Vector3(pos.x, 0f, pos.z + 0.5f));
            }
    ...
    private Vector3 getStartPosAccordingToName() {
        switch (gameObject.name) {
            case "blinky":
                Debug.Log("Case blinky start position");
                return new Vector3(-0.045f, 0.0275f, 1.35625f);
    ...
    void Init() {
        _timeToWhite = 0;
        Debug.Log("0. Init(), and waypoints count = " + waypoints.Count);
    ...
    void Scatter() {
        Debug.Log("Scatter() in ghost move");
    ...
    void ChaseAI() {
        ...
        if (Vector3.Distance(transform.position, waypoint) > 0.000000000001) {
            Vector3 p = Vector3.MoveTowards(transform.position, waypoint, speed);
    ...
    void MoveToWaypoint(bool loop = false) {
        Debug.Log("0 part 1 - get next waypoint");
    ...
    public void Frighten() {
        ...
        _timeToWhite = Time.time + 7 * 0.66f;
