...
public class AI : MonoBehaviour {
    ...
    void Awake() {
        manager = GameObject.Find("Map").GetComponent<TileManager>();
    ...
    public void AILogic() {
        // get current tile
        Vector3 currentPos = new Vector3(transform.position.x + 0.499f, 0, transform.position.z + 0.499f);
        currentTile = tiles[manager.Index((int)currentPos.x, (int)currentPos.z)];

        Debug.Log("AI logic, gettiletargetperghost()");

        targetTile = GetTargetTilePerGhost();

        // get the next tile according to direction
        if (ghost.direction.x > 0) nextTile = tiles[manager.Index((int)(currentPos.x + 1), (int)currentPos.z)];
        if (ghost.direction.x < 0) nextTile = tiles[manager.Index((int)(currentPos.x - 1), (int)currentPos.z)];
        if (ghost.direction.z > 0) nextTile = tiles[manager.Index((int)currentPos.x, (int)(currentPos.z + 1))];
        if (ghost.direction.z < 0) nextTile = tiles[manager.Index((int)currentPos.x, (int)(currentPos.z - 1))];
        ...
        if (nextTile.occupied || currentTile.isIntersection) {
            ...
            if (currentTile.isIntersection) {
                ...
                if (currentTile.up != null && !currentTile.up.occupied && !(ghost.direction.z < 0)) dist1 = manager.distance(currentTile.up, targetTile);
                if (currentTile.down != null && !currentTile.down.occupied && !(ghost.direction.z > 0)) dist2 = manager.distance(currentTile.down, targetTile);
    ...
    public void RunLogic() {
        // get current tile
        Vector3 currentPos = new Vector3(transform.position.x + 0.499f, transform.position.z + 0.499f);
        currentTile = tiles[manager.Index((int)currentPos.x, (int)currentPos.z)];
        ...
        if (nextTile.occupied || currentTile.isIntersection) {
            ...
            if (currentTile.isIntersection) {
                ...
                ghost.direction = Vector3.Normalize(new Vector3(chosenTile.x - currentTile.x, 0, chosenTile.y - currentTile.y));
    ...
    TileManager.Tile GetTargetTilePerGhost() {
        ...
        Debug.Log("Check name Get target tile per ghost");
        switch (name) {
            case "blinky":  // target = pacman
                Debug.Log("get target tile per ghost on blinky");
                targetPos = new Vector3(target.position.x + 0.499f, 0, target.position.z + 0.499f);
                targetTile = tiles[manager.Index((int)targetPos.x, (int)targetPos.z)];
                break;
            case "pinky":   // target = pacman + 4*pacman's direction (4 steps ahead of pacman)
                dir = target.GetComponent<Control>().getDir();
                targetPos = new Vector3(target.position.x + 0.499f, 0, target.position.z + 0.499f) + 4 * dir;
            ...
            case "inky":    // target = ambushVector(pacman+2 - blinky) added to pacman+2
                ...
                targetPos = new Vector3(target.position.x + 0.499f, 0, target.position.z + 0.499f) + 2 * dir + ambushVector;
                targetTile = tiles[manager.Index((int)targetPos.x, (int)targetPos.z)];
                break;
            case "clyde":
                targetPos = new Vector3(target.position.x + 0.499f, 0, target.position.z + 0.499f);
